      ...
      {chartData.length > 0 && (
        <div className="glass-card p-4">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-1.5 max-h-[120px] overflow-y-auto hide-scrollbar">
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        {encendidos.length === 0 ? (
          <div className="glass-card p-4 text-center text-slate-500 text-xs">
            ...
            <button onClick={() => setPhoneScreen('crud')} className="text-emerald-400 font-bold mt-1 hover:underline">
              Ir al catálogo para encender
            </button>
          </div>
        ) : (
          <div className="overflow-y-auto flex-1 min-h-0 space-y-1.5 pr-1 hide-scrollbar">
            {encendidos.map(device => (
              <div key={device.id} className="flex items-center justify-between p-2.5 bg-slate-950/80 border border-slate-800/80 rounded-xl hover:border-emerald-500/30 transition group">
                <div>
